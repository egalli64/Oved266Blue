...
begin
    ...
    (select u.first_name from users u join boats b using(user_id) --return the first name of the new user
    join moorings m using(boat_id) where mooring_id = :new.mooring_id),
    
    (select u.last_name from users u join boats b using(user_id) --return the last name of the new user
    join moorings m using(boat_id) where mooring_id = :new.mooring_id),
    
    (select u.id_number from users u join boats b using(user_id) --return the id_number of the new user
    join moorings m using(boat_id) where mooring_id = :new.mooring_id),
    
    ('New user with boat ' || 
    (select boat_id from boats b join moorings m using(boat_id) where mooring_id = :new.mooring_id) || 
    ' in mooring: ' ||
    :new.mooring_id));
